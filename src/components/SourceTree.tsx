...
export const SourceTree: React.FC<SourceTreeProps> = ({
  ...
  // Derive active view from URL pathname, matching the same convention as App.tsx
  const activeNavView = location.pathname.replace(/^\//, '') || 'home';
  const [activeTab, setActiveTab] = useState<'NAV' | 'DATA'>(mode === 'tree' ? 'DATA' : 'NAV');
  ...
  const navItems = [
    ...
    { id: 'events', label: language === 'VN' ? 'Sự kiện' : 'Events', icon: Calendar },
  ...
  if (mode === 'tree') {
    return (
      <div className="flex-1 w-full bg-white border border-[#DCE1E6] rounded-xl p-4 select-none overflow-y-auto min-h-[300px]">
        <div className="flex items-center justify-between mb-4 pb-2 border-b border-[#EEF1F4]">
          <span className="text-[11px] uppercase tracking-widest text-[#1b4b66] font-mono font-bold flex items-center gap-1.5">
            <Layers className="w-4 h-4 text-[#5B4B8A]" />
            {t.sourceTreeHeader}
          </span>
          <span className="text-[10px] font-mono text-[#72787e] bg-[#F8FAFC] px-2 py-0.5 rounded border border-[#DCE1E6]">
            {treeData.length} Depts
          </span>
        </div>

        {loading && (
          <p className="text-xs text-[#72787e] italic">Đang tải...</p>
        )}
        ...
        {!loading && !error && treeData.length === 0 && (
          <p className="text-xs text-[#72787e] italic">Không có dữ liệu</p>
        )}
                          ...
                          <div key={yearItem.year} className="w-full">
                            ...
                            >
                              <div className="flex items-center gap-1.5 overflow-hidden">
                                ...
                                <span className="truncate text-[11px] font-medium font-mono">Năm {yearItem.year}</span>
                              </div>
                                      ...
                                      {isQtrExpanded && qtrItem.events && (
                                        <div className="pl-3 ml-2 border-l border-[#EEF1F4] space-y-0.5">
                                          {qtrItem.events.map((evt: any) => {
                                            const isSelected = selectedEventIds.includes(evt.eventId) || selectedSourceId === evt.eventId;
                                            return (
                                              <div
                                                key={evt.eventId}
  ...
  return (
    <aside className="w-[240px] lg:w-[260px] bg-[#EEF1F4] border-r border-[#DCE1E6] flex flex-col h-full select-none shrink-0 overflow-hidden transition-colors">
      {/* Tab content forced to NAV, Tab switcher completely removed */}
