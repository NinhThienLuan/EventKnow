...
export const EventDetailView: React.FC<EventDetailViewProps> = ({
    ...
    // Tabs state
    const [activeTab, setActiveTab] = useState<'roster' | 'recommendations'>('roster');
    ...
    useEffect(() => {
        if (activeTab === 'recommendations' && (!detail?.topicTags || detail.topicTags.length === 0 || isEditingTags)) {
            fetchPopularTags()
                .then(tags => setPopularTags(tags))
                .catch(err => console.error("Error loading popular tags:", err));
        }
    }, [activeTab, detail, isEditingTags]);

    // Trigger loading recommendations when tab / minOverlap changes
    useEffect(() => {
        if (activeTab === 'recommendations' && detail?.topicTags && detail.topicTags.length > 0) {
            loadRecommendations(0, true);
        }
    }, [activeTab, minOverlap, detail?.topicTags]);
    ...
    return (
        <div className="space-y-6 max-w-[1400px] mx-auto w-full animate-fade-in text-[#252a31]">
            ...
            {/* Tabs Selector */}
            <div className="flex border-b border-[#DCE1E6] gap-6 text-sm font-semibold pb-1">
                <button
                    onClick={() => setActiveTab('roster')}
                    className={`pb-3 px-1 transition-colors cursor-pointer border-b-2 font-bold ${activeTab === 'roster'
                ...
                <button
                    ...
                    className={`pb-3 px-1 transition-colors cursor-pointer border-b-2 font-bold ${activeTab === 'recommendations'
                        ? 'border-[#1b4b66] text-[#00344c]'
                        : 'border-transparent text-[#72787e] hover:text-[#00344c]'
                        }`}
                >
